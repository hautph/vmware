    ...
    for (const block of questionBlocks) {
        ...
        for (let i = 0; i < cleanLines.length; i++) {
            ...
            if (optionMatch) {
                ...
                // But only if this line doesn't start with a number followed by a period (next option)
                if (!line.match(/^\d+\./)) {
                    currentOption.text += ' ' + line;
                } else {
                    // This is the start of a new option
                    options.push(currentOption);
                    const nextOptionMatch = line.match(/^(\d+)\.\s*(.*)/);
                    if (nextOptionMatch) {
                        const optionNumber = nextOptionMatch[1];
                        const optionText = nextOptionMatch[2].trim();
                        currentOption = {
                            number: optionNumber,
                            letter: String.fromCharCode(64 + parseInt(optionNumber)),
                            text: optionText
                        };
                    }
                }
